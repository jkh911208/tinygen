...
async def generate(request: GenRequest):
    ...
    if not await git.verify_access():
        raise HTTPException(
            status_code=400, detail="Failed to verify access to the git repository."
        )
        ...
        try:
            messages.extend(
                [
                    ...
                    {
                        "role": "user",
                        "content": "Now, critically review your own work. Ensure that the code modifications are not only syntactically correct but also logically sound and align with the user's original request. Verify that the code is executable and free of new bugs.",
                    },
                ]
            )
            modified_files_dict = orjson.loads(content_str)
            messages[-1]["content"] = (
                "Your previous response was successfully parsed as a JSON object. "
                + messages[-1]["content"]
            )
        except orjson.JSONDecodeError as e:
            print(
                f"First attempt JSON parsing failed: {e}. Attempting self-correction..."
            )
            # Self-correction attempt
            messages[-1]["content"] = (
                "The previous response was not a valid JSON object. Please provide the complete and correct JSON object as specified in the system prompt. Do not include any other text or explanations. "
                + messages[-1]["content"]
            )
